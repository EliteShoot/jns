...
app.post("/api/chauffeur-request", async (req, res) => {
  try {
    ...
    console.error("Chauffeur error:", err);

    res.status(200).json({
      success: true,
      warning: "Email failed but request received"
    });
  }
});
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x00FFD1)
      .addFields(
        { name: "Subject", value: name || "Unknown" },
...
client.on("messageCreate", async (message) => {
  ...
  const sessionId = Object.keys(sessions).find(
    id => sessions[id].channelId === message.channel.id
  );
